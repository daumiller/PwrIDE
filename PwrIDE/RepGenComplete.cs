...
namespace PwrIDE
{
  public class RepGenComplete : ICompletionDataProvider
  {
    //========================================================================
    ...
    public static void LoadRecords(string contents)
    {
      ...
      while(i < lines.Length)
      {
        ...
        if(match.Success)
        {
          ...
          Records.Add(valu, LoadFields(regField, lines, ref i));
        }
        else
          i++;
      }
      ...
      for(int i=0; i<words.Length; i++)
      {
        ...
        if(Records.ContainsKey(combined))
        {
          foreach(KeyValuePair<string, string> field in Records[combined])
            icd.Add(new DefaultCompletionData(field.Key, field.Value, 1));
          break;
        }
      }
