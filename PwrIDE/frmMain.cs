...
namespace PwrIDE
{
  public partial class frmMain : Form
  {
    ...
    //	Compiling Stuffs
    //=========================================================================
    public delegate void CompileError(PwrError.ErrorDesc error);
    //------------------------------------------------------------------------
    public delegate void CompileException(string message); 
    //------------------------------------------------------------------------
    public void onCompileError(PwrError.ErrorDesc err)
    {
    	if(ActiveSource != null)
    		ActiveSource.SetPwrError(err);
		}
		//------------------------------------------------------------------------
		public void onCompileException(string desc)
		{
			MessageBox.Show(desc, "PwrIDE", MessageBoxButtons.OK, MessageBoxIcon.Error);
		}
    private void ProcessError(string line)
    {
      Regex getFileAndLine = new Regex(@"@PWRPLUSLINE@([^:]+)::([0-9]+)");
      Match gotFileAndLine = getFileAndLine.Match(line);
      if(gotFileAndLine.Success)
      {
        PwrError.Fatal.Filename = gotFileAndLine.Groups[1].Value;
        PwrError.Fatal.Line = int.Parse(gotFileAndLine.Groups[2].Value)+1;
      }
    }
    //------------------------------------------------------------------------
    public string Compile(string source, string filename, CompileError onError, CompileException onException, Importer importer)
    {
    	string output = null;

      string input = PreProcessor.PreProcess(source, filename, importer.Import);
      if(input == null)
      {
        PwrError.Fatal.Filename = filename;
        onError(PwrError.Fatal);
        return null;
      }
      string[] errLines = input.Split(new char[]{'\n'});

    	Scanner pwrScan = new Scanner();
    	try
    	{
    		if(pwrScan.Scan(input) == false)
    		{
          ProcessError(errLines[PwrError.Fatal.Line-1]);
    			onError(PwrError.Fatal);
    			return null;
    		}
    	}
    	...
    	{
    		onException("Scanner Exception Error\n"+ex.Message);
    		return null;
    	}
    	
    	Parser pwrParse = new Parser(pwrScan);
    	try
    	{
    		if(pwrParse.Parse() == false)
    		{
          ProcessError(errLines[PwrError.Fatal.Line-1]);
    			onError(PwrError.Fatal);
    			return null;
    	...
    	{
    		onException("Parser Exception Error\n"+ex.Message);
    		return null;
    	}
    	
    	Generator pwrGen = new Generator(pwrParse);
    	try
    	{
    		if(pwrGen.Generate() == false)
    		{
          ProcessError(errLines[PwrError.Fatal.Line-1]);
    			onError(PwrError.Fatal);
    			return null;
    	...
    	{
    		onException("Generator Exception Error\n"+ex.Message);
    		return null;
    	}
    	
    	Beautifier pwrBeaut = new Beautifier();
    	try
    	{
    		output = pwrBeaut.Beautify(pwrGen.Output);
    	}
    	catch(Exception ex)
    	{
    		onException("Beautifier Exception Error\n"+ex.Message);
    		return null;
    	}
		...
		//------------------------------------------------------------------------
    private void mnuUtilCompile_Click(object sender, EventArgs e)
    {
    	if(ActiveSource == null) return;
    	if(ActiveSource.fileType != ProjectFile.FileType.PWRPLS) return;

    	string inp  = ActiveSource.icsEditor.Text;
    	string outp = Compile(inp, ActiveSource.fileName, onCompileError, onCompileException, ActiveSource.GetImporter());
    	if(outp == null)
    		return;
    	frmSource update=null;

      if(ActiveSource.fileOrigin == frmSource.Origin.PROJECT)
      {
        ProjectFile prjFile = ActiveSource.fileProject.Parent.AddCompiledSource(ActiveSource.fileProject, outp);
        if(prjFile != null)
        {
        	Explorer.ProjectUpdateSave(ActiveSource.fileProject.Parent);
          update=null;
          IDockContent[] docs = dockPanel.DocumentsToArray();
          for(int i=0; i<docs.Length; i++)
          {
          ...
          if(update == null)
          {
            update = new frmSource(prjFile);
            update.Show(dockPanel);
        ...
        {
          update = new frmSource(ActiveSource.fileName, outp);
          update.Show(dockPanel);
          update.icsEditor.Focus();
        }
      }
